...
public class ServerServiceWindow extends Application {
    ...
    public void start(Stage primaryStage) throws FileNotFoundException {
        ...
        Assets.serverController = new ServerController("Server");
        initiate();
    }

    /**Инициализация, загрузк настроект и так далее*/
    private void initiate() throws FileNotFoundException {
        Assets.users = new Users();
        GsonBuilder builder = new GsonBuilder();
        builder.registerTypeAdapter(Users.class, new Users.UsersConverter());
        Gson gson = builder.create();

//        users.userList.add(new UserCard(1,"admin","forever","Andrei","none", UserCard.Permission.ServerAdmin));
//        users.userList.add(new UserCard(2,"driver","move","Sergei","none", UserCard.Permission.MobileDriver));
//        users.userList.add(new UserCard(3,"dispatcher","for","Vania","none", UserCard.Permission.ServerDispether));
//
//        String JsonTest = gson.toJson(users);
//        Logs.out(JsonTest);
//        ReadWrite.writeJson("users.json",JsonTest);

        String newJson = ReadWrite.read("users.json");
        Assets.users = gson.fromJson(newJson,Users.class);
        Logs.out(gson.toJson(Assets.users));
    }
